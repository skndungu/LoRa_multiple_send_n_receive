...
//0-1dB
float normalizeMicdata(int32_t mic_value) {
  int32_t micVal = mic_value / 100000000;
  //int32_t audio = constrain(micVal,-1000,1000);

  float mapped = map(micVal, -3, 1, 0, 1);
...
float mapMicVal(int32_t micval) {
  //get min and max values dynamically
  int32_t minmicval = -4047483648;
  int32_t maxmicval = -147483648;


  float value = map(micval, minmicval, maxmicval, 0, 1);
  return value;
}
